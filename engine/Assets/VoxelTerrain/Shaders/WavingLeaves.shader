...
////////// start bending
	...
	float3 worldPos = mul((float3x4)_Object2World,v.vertex.xyz);
...
float dist =distance(float3(worldPos.x,worldPos.y,worldPos.z),float3(_Obstacle.x,_Obstacle.y,_Obstacle.z));
...
float distMulti = distLimit-min(distLimit,dist); //distance falloff
	...
	waveMove.x = dot (s, (_waveXSizeMove*WindDirection.x))+ dot((bendDir.x * distMulti),0.5);
	waveMove.y = dot (s, (_waveYSizeMove*WindDirection.y))+ dot((bendDir.x * distMulti),0.5);
	waveMove.z = dot (s, (_waveZSizeMove*WindDirection.z))+dot(( bendDir.z * distMulti),0.5);
